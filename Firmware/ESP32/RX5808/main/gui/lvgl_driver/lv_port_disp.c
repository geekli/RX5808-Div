...
static void IRAM_ATTR disp_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
    ...
    memset(&spi_tr, 0, sizeof(spi_tr));       // Zero out the transaction
    spi_tr.length = width * height * 16; // Command is 8 bits
    // t.user=(void*)1;                //D/C needs to be set to 0
    spi_tr.tx_buffer = color_p;
    gpio_set_level(PIN_NUM_DC, 1);

    ret = spi_device_polling_transmit(my_spi, &spi_tr); // Transmit!
